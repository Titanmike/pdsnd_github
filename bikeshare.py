...
def get_filters():
    ...
    """
    print('\n Hello! My name is Bolatito Osiname \n Let\'s explore some US bikeshare data!')
    ...
    while True:
        ...
        if city in cities:
            ...
            print("\n Sorry we were not able to get the name of the city to filter data, \n Please enter a valid city name")


    # get user input for month (january, february, ... , june or none)
    while True:
        months= ['January','February','March','April','June','May','None']
        month = input("\n Which month would you like to consider? (Jan, Feb, Mar, Apr, May, Jun)? Type 'None' for no month filter\n").title()
        if month in months:
            break
        else:
            print("\n Sorry we were not able to get the name of the month to filter data,\n  Please enter a valid month")
    ...
    while True:
        ...
        day = input("\n Which day of the week would you like to consider? (Mon, Tue, Wed, Thur, Fri, Sat, Sun)? Type 'None' for no day filter \n").title()
        if day in days:
            break
        else:
            print("\n Sorry we were not able to get the name of the day to filter data,\n  Please enter a valid day")
...
def time_stats(df,month,day):
    ...
    if month =='None':
        highest_occuring_month= df['month'].mode()[0]
        months= ['January','February','March','April','May','June']
        highest_occuring_month= months[highest_occuring_month-1]
        print("The most Popular month is",highest_occuring_month)


    # display the most common day of week
    if day =='None':
        highest_occuring_day= df['day_of_week'].mode()[0]
        print("The most Popular day is",highest_occuring_day)


    # display the most common start hour
    df['Start Hour'] = df['Start Time'].dt.hour
    highest_occuring_hour=df['Start Hour'].mode()[0]
    print("The popular Start Hour is {}:00 hrs".format(highest_occuring_hour))
